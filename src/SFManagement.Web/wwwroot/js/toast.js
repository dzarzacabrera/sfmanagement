(function () {
    ...
    window.showToast = function (message, type) {
        ...
        var colors = {
            ...
            undo: 'bg-white text-gray-900 border border-gray-200'
        };
        var icons = {
            ...
            undo: '<svg class="w-4 h-4 shrink-0 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>'
        };
    ...
    window.showUndoToast = function (message, onUndo) {
        var toast = document.createElement('div');
        toast.className = 'pointer-events-auto flex items-center gap-2 px-4 py-3 rounded-lg shadow-lg text-sm font-medium bg-white text-gray-900 border border-gray-200 translate-x-full opacity-0 transition-all duration-300';
        toast.innerHTML = '<svg class="w-4 h-4 shrink-0 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><span>' + message + '</span><button class="ml-2 px-2 py-1 rounded bg-brand text-white hover:bg-brand-deep font-semibold text-sm transition-colors">Undo</button>';
        container.appendChild(toast);
