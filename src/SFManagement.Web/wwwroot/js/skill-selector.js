window.validateSkillSelection = function (form) {
    if (form.querySelectorAll('input[name="skillPositions"]').length === 0) {
        window.showToast('Please select at least one required skill.', 'error');
        return false;
    }
    return true;
};

window.initSkillSelectors = function (container) {
    var scope = container || document;
    scope.querySelectorAll('.skill-selector').forEach(function (container) {
        ...
        function render() {
            ...
            var selectedSection = container.querySelector('.skill-selected-section');
            if (selectedSection) {
                selectedSection.classList.toggle('hidden', selected.size === 0);
            }
        ...
        unselectedBox.addEventListener('click', function (e) {
            ...
            selected.set(pos, 6);
            render();
            var items = container.querySelectorAll('.skill-selected-item');
