...
5.  **Estados de las Tareas (Kanban Core):** El sistema debe manejar de forma obligatoria y estricta el ciclo de vida de las tareas reflejado en 4 estados en la base de datos: `Queued`, `In Progress`, `Blocked` y `Finish`. El Dashboard renderizará estos estados en 4 columnas responsivas. Las acciones para actualizar el estado o abrir el modal de asignación de un `Worker` deben ejecutarse mediante Vanilla JS enviando peticiones asíncronas hacia los Command Handlers del Backend.
